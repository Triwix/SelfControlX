...
int main(int argc, char* argv[]) {
    [SCSentry startSentry: @"org.eyebeam.selfcontrolx-cli"];
    ...
    @autoreleasepool {
		...
		if([arguments booleanValueForSignature: startSig]) {
            ...
            if ([SCBlockUtilities anyBlockIsRunning]) {
	            ...
	            NSArray* blocklist;
	            NSDate* blockEndDate;
	            NSTimeInterval requestedDurationSecs = 0;
            ...
            if (pathToBlocklistFile != nil && blockEndDateArg != nil && [blockEndDateArg timeIntervalSinceNow] >= 1) {
                ...
                blockAsWhitelist = [readProperties[@"BlockAsWhitelist"] boolValue];
	                ...
	                requestedDurationSecs = MAX([defaultsDict[@"BlockDuration"] intValue] * 60, 0);
	                blockEndDate = [NSDate dateWithTimeIntervalSinceNow: requestedDurationSecs];
	            }
	            
	            // read in the other block settings, starting with defaults
	                NSInteger maxBlockLengthMinutes = [defaultsDict[@"MaxBlockLength"] integerValue];
	                maxBlockLengthMinutes = MIN(MAX(maxBlockLengthMinutes, 1), 10080);
	                BOOL blockBypassesEnabled = defaultsDict[@"BlockBypassesEnabled"] == nil ? YES : [defaultsDict[@"BlockBypassesEnabled"] boolValue];
                id trustedTimeSourceURLs = defaultsDict[@"TrustedTimeSourceURLs"] ?: SCConstants.defaultUserDefaults[@"TrustedTimeSourceURLs"];
                id trustedTimeConsensusRequiredCount = defaultsDict[@"TrustedTimeConsensusRequiredCount"] ?: SCConstants.defaultUserDefaults[@"TrustedTimeConsensusRequiredCount"];
                id trustedTimeConsensusMaxSkewSeconds = defaultsDict[@"TrustedTimeConsensusMaxSkewSeconds"] ?: SCConstants.defaultUserDefaults[@"TrustedTimeConsensusMaxSkewSeconds"];
		            ...
		            NSDictionary* blockSettingsFromDefaults = @{
	                    ...
	                    @"BlockBypassesEnabled": @(blockBypassesEnabled),
	                    ...
	                    @"MaxBlockLengthMinutes": @(maxBlockLengthMinutes),
		                @"RequestedDurationSeconds": @(requestedDurationSecs)
		            };
			...
			if([fileManager fileExistsAtPath: @"/Library/PrivilegedHelperTools/org.eyebeam.SelfControlX"]) {
				if(![fileManager removeItemAtPath: @"/Library/PrivilegedHelperTools/org.eyebeam.SelfControlX" error: nil]) {
