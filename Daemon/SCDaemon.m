...
static NSString* serviceName = @"org.eyebeam.selfcontrolxxd";
...
- (void)start {
    ...
    self.hostsFileWatcher = [SCFileWatcher watcherWithFile: @"/etc/hosts" block:^(NSError * _Nonnull error) {
        if ([SCBlockUtilities anyBlockIsRunning]) {
            NSLog(@"INFO: hosts file changed, re-enforcing block rules.");
            [SCDaemonBlockMethods reapplyBlockRulesAfterHostsMutation];
        }
    }];
}
...
- (BOOL)listener:(NSXPCListener *)listener shouldAcceptNewConnection:(NSXPCConnection *)newConnection {
    ...
    SecRequirementCreateWithString(CFSTR("anchor apple generic and (identifier \"org.eyebeam.SelfControlX\" or identifier \"org.eyebeam.selfcontrolx-cli\") and info [CFBundleVersion] >= \"407\""), kSecCSDefaultFlags, &isSelfControlApp);
