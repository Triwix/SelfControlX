...
NSString* const kPFAnchorCommand = @"anchor \"org.eyebeam.selfcontrolx\"";
...
- (void)writeConfiguration {
	...
	[filterConfiguration writeToFile: @"/etc/pf.anchors/org.eyebeam.selfcontrolx" atomically: true encoding: NSUTF8StringEncoding error: nil];
}
...
- (void)enterAppendMode {
    ...
    appendFileHandle = [NSFileHandle fileHandleForWritingAtPath: @"/etc/pf.anchors/org.eyebeam.selfcontrolx"];
    if (!appendFileHandle) {
...
- (void)appendRulesToCurrentBlockConfiguration:(NSArray<NSDictionary*>*)newEntryDicts {
    ...
    NSFileHandle* fileHandle = [NSFileHandle fileHandleForWritingAtPath: @"/etc/pf.anchors/org.eyebeam.selfcontrolx"];
    if (!fileHandle) {
        NSLog(@"ERROR: Failed to get handle for pf.anchors file while attempting to append rules");
        return;
    }
...
- (void)writePFToken:(NSString*)token error:(NSError**)error {
	[token writeToFile: @"/etc/SelfControlXPFToken" atomically: YES encoding: NSUTF8StringEncoding error: error];
}
- (NSString*)readPFToken:(NSError**)error {
	return [NSString stringWithContentsOfFile: @"/etc/SelfControlXPFToken" encoding: NSUTF8StringEncoding error: error];
}
...
- (int)stopBlock:(BOOL)force {
	...
	[@"" writeToFile: @"/etc/pf.anchors/org.eyebeam.selfcontrolx" atomically: true encoding: NSUTF8StringEncoding error: nil];
	...
	NSMutableString* newConf = [NSMutableString stringWithCapacity: [mainConf length]];
		for (NSString* line in lines) {
			if ([line rangeOfString: @"org.eyebeam.selfcontrolx"].location == NSNotFound) {
...
- (void)addSelfControlConfig {
	NSMutableString* pfConf = [NSMutableString stringWithContentsOfFile: @"/etc/pf.conf" encoding: NSUTF8StringEncoding error: nil];
		...
		if ([pfConf rangeOfString: @"/etc/pf.anchors/org.eyebeam.selfcontrolx"].location == NSNotFound) {
			[pfConf appendString: @"\n"
			 "anchor \"org.eyebeam.selfcontrolx\"\n"
			 "load anchor \"org.eyebeam.selfcontrolx\" from \"/etc/pf.anchors/org.eyebeam.selfcontrolx\"\n"];
		}
...
- (BOOL)containsSelfControlBlock {
	NSString* mainConf = [NSString stringWithContentsOfFile: @"/etc/pf.conf" encoding: NSUTF8StringEncoding error: nil];
	return mainConf != nil && [mainConf rangeOfString: @"org.eyebeam.selfcontrolx"].location != NSNotFound;
}
