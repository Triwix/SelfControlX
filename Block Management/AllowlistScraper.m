...
+ (NSSet<SCBlockEntry*>*)relatedBlockEntries:(NSString*)domain; {
	...
	NSURLSessionDataTask* fetchTask = [session dataTaskWithRequest: request
	...
	[fetchTask resume];
	long waitResult = dispatch_semaphore_wait(fetchSemaphore, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(6 * NSEC_PER_SEC)));
	if (waitResult != 0) {
		[fetchTask cancel];
