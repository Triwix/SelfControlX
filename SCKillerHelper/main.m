...
int main(int argc, char* argv[]) {
	@autoreleasepool {
        [SCSentry startSentry: @"com.selfcontrolapp.SCKillerHelperX"];
		...
		args = @[@"read", @"org.eyebeam.SelfControlX"];
		[task setArguments:args];
		...
		if([fileManager fileExistsAtPath: @"/etc/pf.anchors/org.eyebeam.selfcontrolx"]) {
			[log appendString: [NSString stringWithFormat: @"Found anchor file with contents:\n\n%@\n\n", [NSString stringWithContentsOfFile: @"/etc/pf.anchors/org.eyebeam.selfcontrolx" encoding: NSUTF8StringEncoding error: nil]]];
		}
		...
		task = [NSTask launchedTaskWithLaunchPath: @"/bin/launchctl"
										arguments: @[@"unload",
                                                     @"-w",
													 @"/Library/LaunchDaemons/org.eyebeam.SelfControlX.plist"]];
		[task waitUntilExit];
        status = [task terminationStatus];
        ...
        for (NSString* daemonLabel in daemonLabelsToRemove) {
            ...
            if (cfError) {
                [log appendFormat: @"Failed to remove selfcontrold daemon label %@ with error %@\n\n", daemonLabel, cfError];
                CFRelease(cfError);
            } else {
                [log appendFormat: @"Successfully removed selfcontrold daemon label %@!\n\n", daemonLabel];
            }
        }
		...
		// remove PF token
		if([fileManager removeItemAtPath: @"/etc/SelfControlXPFToken" error: nil]) {
		...
		if([fileManager fileExistsAtPath: @"/etc/pf.anchors/org.eyebeam.selfcontrolx"]) {
			if([fileManager removeItemAtPath: @"/etc/pf.anchors/org.eyebeam.selfcontrolx" error: nil])
