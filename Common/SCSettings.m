    ...
    if (self = [super init]) {
        ...
        [[NSDistributedNotificationCenter defaultCenter] addObserver: self
                                                            selector: @selector(onSettingChanged:)
                                                                name: @"org.eyebeam.SelfControlX.SCSettingsValueChanged"
                                                              object: nil
...
+ (NSString*)settingsFileName {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        fileName = [NSString stringWithFormat: @".%@.plist", [SCMiscUtilities sha1: [NSString stringWithFormat: @"SelfControlXUserPreferences%@", [SCMiscUtilities getSerialNumber]]]];
    });
...
- (NSDictionary*)defaultSettingsDict {
    return @{
        ...
        // trusted time settings used to resist local clock manipulation
        @"TrustedTimeEnforced": @NO,
        @"TrustedBlockEndDate": [NSDate distantPast],
        @"TrustedTimeLastFetchDate": [NSDate distantPast],
        @"TrustedTimeLastFetchUptime": @0,
...
- (void)setValue:(id)value forKey:(NSString*)key stopPropagation:(BOOL)stopPropagation {
    ...
    if (!stopPropagation) {
        [[NSDistributedNotificationCenter defaultCenter] postNotificationName: @"org.eyebeam.SelfControlX.SCSettingsValueChanged"
