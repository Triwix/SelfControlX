...
+ (NSString*)canonicalDaemonJobLabel {
    return @"org.eyebeam.selfcontrolxxd";
}
...
- (void)connectToHelperTool {
    ...
    if (self.daemonConnection == nil) {
        self.daemonConnection = [[NSXPCConnection alloc] initWithMachServiceName: [SCXPCClient canonicalDaemonJobLabel] options: NSXPCConnectionPrivileged];
...
- (void)installDaemon:(void(^)(NSError*))callback {
    ...
    AuthorizationItem startBlockRight = {
        "org.eyebeam.SelfControlX.startBlock", 0, NULL, 0
    };
    ...
    NSArray<NSString *> *daemonLabelsToRemove = [SCXPCClient daemonLabelsForMigrationCleanup];
    for (NSString *daemonLabel in daemonLabelsToRemove) {
        SILENCE_OSX10_10_DEPRECATION(
        SMJobRemove(kSMDomainSystemLaunchd, (__bridge CFStringRef)daemonLabel, self->_authRef, YES, &cfError);
                                     );
        if (cfError) {
            NSLog(@"WARNING: Failed to remove existing selfcontrold daemon (%@) with error %@", daemonLabel, cfError);
            CFRelease(cfError);
            cfError = NULL;
        }
    }
    ...
    BOOL result = (BOOL)SMJobBless(
                                   ...
                                   (__bridge CFStringRef)[SCXPCClient canonicalDaemonJobLabel],
                                   self->_authRef,
                                   &cfError);
...
- (NSError*)authorizeDaemonCommand:(SEL)command {
    ...
    void (^authBlock)(void) = ^{
        ...
        if (self->_authRef == NULL) {
            retErr = [NSError errorWithDomain: NSOSStatusErrorDomain code: paramErr userInfo: nil];
            return;
        }
        
        NSString* rightName = [SCXPCAuthorization authorizationRightForCommand: command];
        if (rightName.length < 1) {
            return;
        }
        
        AuthorizationItem right = {
            [rightName UTF8String], 0, NULL, 0
        };
        AuthorizationRights authRights = {
            1, &right
        };
        ...
        if (status != errAuthorizationSuccess) {
            retErr = [NSError errorWithDomain: NSOSStatusErrorDomain code: status userInfo: nil];
            return;
        }
        
        // Refresh external-form auth data after obtaining additional rights.
        [self updateStoredAuthorization: self->_authRef];
    };
    
    // SecurityAgent prompts are most reliable when rights are requested from the app on main thread.
    if ([NSThread isMainThread]) {
        authBlock();
    } else {
        dispatch_sync(dispatch_get_main_queue(), ^{
...
- (void)updateBlocklist:(NSArray<NSString*>*)newBlocklist reply:(void(^)(NSError* error))reply {
    NSError* authError = [self authorizeDaemonCommand: @selector(updateBlocklist:authorization:reply:)];
    if (authError != nil) {
        if (![SCMiscUtilities errorIsAuthCanceled: authError]) {
            NSLog(@"Blocklist update authorization failed with error %@", authError);
            [SCSentry captureError: authError];
        }
        reply(authError);
        return;
    }
...
- (void)updateBlockEndDate:(NSDate*)newEndDate reply:(void(^)(NSError* error))reply {
    NSError* authError = [self authorizeDaemonCommand: @selector(updateBlockEndDate:authorization:reply:)];
    if (authError != nil) {
        if (![SCMiscUtilities errorIsAuthCanceled: authError]) {
            NSLog(@"Block end date update authorization failed with error %@", authError);
            [SCSentry captureError: authError];
        }
        reply(authError);
        return;
...
- (void)forceClearBlock:(void(^)(NSError* error))reply {
    NSError* authError = [self authorizeDaemonCommand: @selector(forceClearBlockWithAuthorization:reply:)];
    if (authError != nil) {
        if (![SCMiscUtilities errorIsAuthCanceled: authError]) {
            NSLog(@"Manual block clear authorization failed with error %@", authError);
            [SCSentry captureError: authError];
        }
        reply(authError);
        return;
    ...
    [self connectAndExecuteCommandBlock:^(NSError * connectError) {
        if (connectError != nil) {
            NSLog(@"Manual block clear failed with connection error: %@", connectError);
            [SCSentry captureError: connectError];
            reply(connectError);
        } else {
            [[self.daemonConnection remoteObjectProxyWithErrorHandler:^(NSError * proxyError) {
                NSLog(@"Manual block clear command failed with remote object proxy error: %@", proxyError);
                [SCSentry captureError: proxyError];
                reply(proxyError);
            }] forceClearBlockWithAuthorization: self.authorization reply:^(NSError* error) {
                if (error != nil && ![SCMiscUtilities errorIsAuthCanceled: error]) {
                    NSLog(@"Manual block clear failed with error = %@\n", error);
                    [SCSentry captureError: error];
                }
                reply(error);
            }];
...
- (NSString*)selfControlHelperToolPath {
    ...
    dispatch_once(&onceToken, ^{
        ...
        path = [thisBundle.bundlePath stringByAppendingFormat: @"/Contents/Library/LaunchServices/%@", [SCXPCClient canonicalDaemonJobLabel]];
    });
